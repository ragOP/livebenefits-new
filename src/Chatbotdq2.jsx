...
export default function Chatbot() {
    ...
    if (typeof window.rbAge !== "function") {
      window.rbAge = function rbAge(value) {
        try {
          ...
          // Newsbreak event (optional, already mirroring to newsbreak_cid above)
          try {
    ...
    if (typeof window.rbPartAB !== "function") {
      window.rbPartAB = function rbPartAB(value) {
        try {
          ...
          // Newsbreak custom event (no CID override here to avoid clobbering age)
          try {
  ...
  useEffect(() => {
    const initialMessages = [
      ...
      {
        text:
          "Emily this side. Let’s find out if you qualify for the Spending Allowance Worth Thousands — it’s quick and only takes 2 minutes!",
        sender: "bot",
